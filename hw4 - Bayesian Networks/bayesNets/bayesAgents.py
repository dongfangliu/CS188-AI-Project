...
def fillYCPT(bayesNet, gameState):
    ...
    """
    from layout import PROB_BOTH_BOTTOM,PROB_BOTH_TOP,PROB_LEFT_TOP
...
def fillObsCPT(bayesNet, gameState):
    ...
    """
    def getAdjHouse(bottomLeftPos, topLeftPos, bottomRightPos, topRightPos,obsPos):
        dist = [util.manhattanDistance(obsPos,pos) for pos in [bottomLeftPos, topLeftPos, bottomRightPos, topRightPos] ]
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            ajacentHouse = getAdjHouse(bottomLeftPos, topLeftPos, bottomRightPos, topRightPos,obsPos)
            for assignment in obsCPT:
                prob = 0
                if(assignment[GHOST_HOUSE_VAR] != ajacentHouse and assignment[FOOD_HOUSE_VAR] != ajacentHouse):
                    if assignment[obsVar]==RED_OBS_VAL:
                        ...
                        prob = 1
                elif assignment[GHOST_HOUSE_VAR] == assignment[FOOD_HOUSE_VAR] or assignment[FOOD_HOUSE_VAR] == ajacentHouse:
                    if assignment[obsVar]==RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assignment[obsVar]==BLUE_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    house_factor = inference.inferenceByVariableElimination(bayesNet,FOOD_HOUSE_VAR,evidence,eliminationOrder)
    best =None
    prob = 0
    for assignment in house_factor.getAllPossibleAssignmentDicts():
        temp_prob = house_factor.getProbability(assignment)
        if temp_prob>prob:
            prob = temp_prob
            best = assignment
    return best
