...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs in enumerate(self.ghostBeliefs) if
                                            livingGhosts[i + 1]]
        mostLikePos = [x.argMax() for x in livingGhostPositionDistributions]

        distances = [self.distancer.getDistance(pacmanPosition, x) for x in mostLikePos]
        nearestPos = mostLikePos[distances.index(min(distances))]
        dist = float('inf')
        action_taken = None
        for action in legal:
            successPos = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successPos, nearestPos) < dist:
                dist = self.distancer.getDistance(successPos, nearestPos)
                action_taken = action
        return action_taken
